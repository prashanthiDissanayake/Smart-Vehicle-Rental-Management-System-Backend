...
export const Addons: React.FC<AddonsProps> = ({
  ...
  const addonsTotal =
    selectedAddons.reduce((sum, a) => sum + a.price, 0) + driverPrice;

  const [isSubmitting, setIsSubmitting] = useState(false);

  const handleContinue = async () => {
    if (dates.pickup > dates.return) {
      alert("Pickup date must be before end date");
      return;
    }
          ...
          >
            <div className="mb-10">
              ...
              <p className="text-slate-500">
                Select your dates, driving preference and any add-ons for your{" "}
                {car?.name}.
              </p>
            </div>
            ...
            <div className="mb-12">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div
                  ...
                  className={`cursor-pointer p-6 rounded-3xl border-2 transition-all flex flex-col items-center gap-3 text-center ${
                    drivingMode === "self"
                ...
                {AVAILABLE_ADDONS.map((addon) => {
                  ...
                  return (
                    <motion.div
                      ...
                      className={`group cursor-pointer p-6 rounded-3xl border-2 transition-all flex items-center gap-6 ${
                        isSelected
                          ? "border-indigo-600 bg-indigo-50/50"
                          : "border-slate-100 hover:border-slate-200 bg-white"
                      }`}
              ...
              <div className="space-y-3 mb-6">
                ...
                <div className="flex justify-between text-sm">
                  <span className="text-slate-500">
                    {drivingMode === "self" ? "Self Drive" : "With Driver"}
                  </span>
                  <span className="font-bold text-slate-900">
                    {drivingMode === "driver" ? "+$50" : "Included"}
                  </span>
                </div>
              </div>
